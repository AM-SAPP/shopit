...
const add2cart = (id) => {
    ...
    let opt = {
        ...
        body : JSON.stringify({grpcart: room, product: id })
    }


    fetch(domain + "/api/addcart", opt)
    .then (Res => {
        console.log(Res);
        ...
        let ppp = document.getElementsByName(id)[0];
        ppp.innerHTML = "Added";
    })
    .catch( eeer => {
        console.log(eeer);
...
socket.on('message',(message) => {
    ...
    let tts = message.text.split(",");

    if(message.username == "Shopit"){
        // let card = document.createElement("div");
        // card.innerHTML = insertCardt(link, "do");
        ...
        const html = Mustache.render( cardTemplate, {
            username: "Shopit",
            product_name: tts[1],
            price: tts[2], // mustache will render that message to the end of div tag
            createdAt: moment(message.createdAt).format('MMMM Do YYYY, h:mm:ss A'),
            img_url : `img/${tts[4]}.jpg`,
            id : tts[4],
            link : tts[3],
            name : tts[4]
        })
        // console.log(html);
        // let ps = html.getElementsByTagName("p");
        // console.log(ps);
